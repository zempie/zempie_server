...
class RoomManager {
    constructor() {
        ...
        this.enter = (ws) => {
            ...
            // 이미 들어가 있는 방이 있으면 에러
            if (room_id) {
                return packetService_1.SendPacket(ws, 'error');
            }
            // 게임별 방 목록
            ...
            _.forEach(room.members, (_ws) => {
                if (_ws !== ws) {
                    packetService_1.SendPacket(_ws, 'enter', { uid: user_uid });
                }
            });
            // 입장한 유저에게 기존 정보 알림
            const members = Object.keys(room.members);
            packetService_1.SendPacket(ws, 'enter_ok', { members });
            // 풀방?
            if (Object.keys(room.members).length === _.toNumber(num)) {
                const members = Object.keys(room.members);
                packetService_1.Broadcast(room.members, ws, 'full', { members });
            }
        };
        this.leave = (ws) => {
            let { uid: user_uid, gameData: { game_id, num, room_id, } } = ws;
            ...
            if (!room) {
                packetService_1.SendPacket(ws, 'error', { message: 'invalid_room_id' });
            }
            // 나가기
        ...
        this.play = (ws, packetData) => {
            let { uid: user_uid, gameData: { game_id, num, room_id, } } = ws;
            if (!room_id) {
                return packetService_1.SendPacket(ws, 'error', { message: '넌 방에 없음' });
            }
            const room = this.getCurrentRoom(game_id, num, room_id);
            // 데이터 저장
    ...
    defaultRouter(room, ws, packetData) {
        ...
        _.forEach(room.members, (_ws) => {
            if (_ws !== ws) {
                packetService_1.SendPacket(_ws, 'play', {
                    from: ws.uid,
                    data: packetData
                });
            }
